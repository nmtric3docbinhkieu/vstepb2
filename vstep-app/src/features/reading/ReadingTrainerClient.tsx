...
import type { ReadingAttempt, ReadingQuestionPublic } from "@/types/reading";
...
export function ReadingTrainerClient() {
    ...
    const loadData = async () => {
      ...
      try {
        const [moduleResponse, attemptsResponse] = await Promise.all([
          fetch("/api/reading/module", { method: "GET" }),
          fetch("/api/reading/submit", { method: "GET" }),
        ]);

        const moduleResult = (await moduleResponse.json()) as ReadingModuleResponse;
        const attemptsResult = (await attemptsResponse.json()) as ReadingAttemptsResponse;

        if (!moduleResponse.ok || !moduleResult.success || !moduleResult.data) {
          setErrorMessage(moduleResult.message || "Unable to load reading module.");
          return;
        }
        ...
        setPassage(moduleResult.data.passage);
        ...
        setErrorMessage("Unable to load reading module.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const submitAnswers = async () => {
    ...
    try {
      ...
      const result = (await response.json()) as ReadingSubmitResponse;

      if (!response.ok || !result.success || !result.data) {
        setErrorMessage(result.message || "Unable to submit reading answers.");
        return;
      }
      ...
      setErrorMessage("Unable to submit reading answers.");
    } finally {
  ...
  return (
    <div className="space-y-6">
      <section className="surface-3d rounded-2xl p-6">
        <h2 className="text-xl font-semibold text-slate-900">Reading Passage</h2>
        {isLoading ? <p className="mt-3 text-sm text-slate-600">Loading reading module...</p> : null}

        {!isLoading ? (
          <>
            <p className="mt-2 text-sm text-slate-700">{title} ({level})</p>
            <p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-slate-800">{passage}</p>
          </>
        ) : null}
      </section>
      ...
      <section className="surface-3d rounded-2xl p-6">
        ...
        {latestAttempt ? (
          ...
          <p className="mt-3 text-sm text-slate-600">Submit answers to view reading score.</p>
        )}
      </section>
